...
test.describe('Performance Benchmarks', () => {
  test('homepage loads within performance budget', async ({ page }) => {
    ...
    console.log('Performance metrics:', { loadTime, metrics });
  });
  ...
  test('draft builder loads within performance budget', async ({ page }) => {
    await page.goto('/draft-builder');
    await page.waitForLoadState('networkidle');
    ...
    if (fcp) {
      // First Contentful Paint should occur within 2 seconds
  ...
  test('bundle size analysis', async ({ page }) => {
    ...
    expect(jsBundleSize).toBeLessThan(1024 * 1024);
    
    console.log('Bundle analysis:', {
      totalResources: resources.length,
      jsBundleSize: Math.round(jsBundleSize / 1024) + 'KB',
      resources: resources.slice(0, 10) // Log first 10 resources
    });
  });
  ...
  test('memory usage monitoring', async ({ page }) => {
    ...
    if (memoryInfo) {
      // Used heap should be reasonable (under 50MB for initial load)
